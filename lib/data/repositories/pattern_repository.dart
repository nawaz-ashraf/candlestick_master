  ...
  Future<List<CandlestickPattern>> getPatterns() async {
    ...
    try {
      ...
      final String response = await rootBundle
          .loadString('assets/patterns.json')
          .timeout(
            const Duration(seconds: 10),
            onTimeout: () => throw TimeoutException('Failed to load patterns: timeout'),
          );
      
      final List<dynamic> data = json.decode(response);
      _cachedPatterns = data.map((json) => CandlestickPattern.fromJson(json)).toList();
      return _cachedPatterns!;
    } on TimeoutException {
